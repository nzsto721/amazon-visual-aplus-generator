...
> 一套 WorkBuddy Skill 跑完亚马逊副图 + A+ 全流程：从市场调研、视觉策划、JSON 脚本到生图裁切。把跨境人的视觉工作流收进一条指令。
...
```bash
# 把整个文件夹放到 WorkBuddy skills 目录
cp -r amazon-visual-aplus-generator ~/.workbuddy/skills/
```

或者直接把 GitHub 仓库 clone 进去：
...
├── SKILL.md                          # WorkBuddy Skill 核心定义（5 阶段工作流 + 约束规则）
├── README.md                         # 本文件
├── .gitignore
├── scripts/
